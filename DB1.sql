CREATE DATABASE PostGradOffice8;
go
use PostGradOffice8;
...
begin
set @id=(select id from PostGradUser where email=@email and password=@password)
set @success =1
...
as
if(not exists(select * from Examiner e where @ExaminerName=e.name and @fieldOfWork=e.fieldOfWork))
begin
...
begin
declare @idd int
select @idd=e.id from Examiner e where @ExaminerName=e.name and @fieldOfWork=e.fieldOfWork
insert into ExaminerEvaluateDefense
values(@DefenseDate,@ThesisSerialNo,@idd,null)
end
go
...
@studentID int
as
select cid,code,grade
from NonGucianStudentTakeCourse ng inner join Course c on ng.cid=c.id
where sid = @studentID
go
...
@studentId int
As
select th.serialNumber,th.field,th.type,th.title,th.startDate,th.endDate,th.defenseDate,th.years,th.grade,th.noOfExtensions 
from GUCianStudentRegisterThesis gs inner join Thesis th on gs.serial_no=th.serialNumber
where gs.sid=@studentId
union
select th.serialNumber,th.field,th.type,th.title,th.startDate,th.endDate,th.defenseDate,th.years,th.grade,th.noOfExtensions 
from Thesis th inner join NonGUCianStudentRegisterThesis ngs on ngs.serial_no=th.serialNumber
where ngs.sid=@studentId

go
create proc ExaminerEditProfile
@examinerID int,
@fieldofwork varchar(100),
@name varchar(20)
As
Update Examiner set fieldOfWork=@fieldofwork,name=@name where id=@examinerID

go 
create proc ExaminerListAllTheses
@examinerID int
AS
select th.serialNumber,th.title,sup.name,gc.firstName,gc.lastName,ex.date from  ExaminerEvaluateDefense ex inner join  Thesis th on ex.serialNo=th.serialNumber inner join GUCianStudentRegisterThesis gcth on gcth.serial_no=th.serialNumber
inner join GucianStudent gc on gc.id=gcth.sid inner join Supervisor sup on  sup.id=gcth.supid
where ex.examinerId=@examinerID
union
select th.serialNumber,th.title,sup.name,gc.firstName,gc.lastName,ex.date from  ExaminerEvaluateDefense ex inner join  Thesis th on ex.serialNo=th.serialNumber inner join NonGUCianStudentRegisterThesis gcth on gcth.serial_no=th.serialNumber
inner join NonGucianStudent gc on gc.id=gcth.sid inner join Supervisor sup on  sup.id=gcth.supid
where ex.examinerId=@examinerID
...
where ((th.title like '%' + @key + '%') or (th.title like '%' + @key) or (th.title like @key +'%') or (th.title = @key))
...
declare @id int
set @id = SCOPE_IDENTITY()
insert into Examiner values(@id,@name,@fieldOfWork,@nationality)
